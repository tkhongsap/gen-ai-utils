...
def process_threads_and_messages(file_path):
    ...
    Only retain messages where the role is 'user'.
    """
    ...
    all_threads_data = []  # To store messages from all threads where role is 'user'
    ...
    for thread_id in thread_ids:
        ...
        if messages_data:
            ...
            for message in messages:
                ...
                # Only include messages where the role is 'user'
                if role == "user":
            ...
            # Store the messages for this thread if any messages were found
            if message_list:
    ...
    output_path = os.path.join(os.path.dirname(file_path), "retrieved_user_messages.json")
